...
__global__ void sgemm_tensor_core_mma_kernel(
    float* C,
    const float* A,
    const float* B,
    int M, int N, int K) {
    ...
    for (int s = BK; s < K; s += BK) {
        ...
        for (int i = 0; i < Warp_M; i++) {
            ...
            uint32_t smem_reg_A = 
                __cvta_generic_to_shared(&A_shared[choice][16 * (i + warp_m * Warp_M) + (lane_id % 16)][(lane_id / 16) * 4]);
        ...
        for (int i = 0; i < Warp_N; i++) {
            ...
            RB[i][0] = (uint32_t&)B_shared[choice][lane_id % 4][8 * (i + warp_n * Warp_N) + (lane_id / 4)];
            RB[i][1] = (uint32_t&)B_shared[choice][lane_id % 4 + 4][8 * (i + warp_n * Warp_N) + (lane_id / 4)];
        }
...
void sgemm_v12_tensor_core_mma(float* C, const float* A, const float* B, const MatrixDims& dims) {
    ...
    sgemm_tensor_core_mma_kernel<<<gridDim, blockDim>>>(C, A, B, dims.M, dims.N, dims.K);
